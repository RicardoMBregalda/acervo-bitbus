...
const buscarVisita = () => {
  axios.get('http://127.0.0.1:8000/api/visita?id=' + buscar.value)
    .then(response => {
      visitas.value = response.data.data;
    })
    .catch(error => {
  ...
  <div class="flex flex-col top-3.5 sm:ml-64 relative overflow-x-auto pr-10">
    ...
    <div class="mx-4 flex space-y-4 sm:space-y-0 justify-start">
      ...
      <div class="relative">
        ...
        <input type="text" id="table-search" v-model="buscar" @keyup.enter="buscarVisita"
        ...
        <button
          ...
          @click="buscarVisita">Buscar</button>
      </div>
    </div>
        ...
        <tbody>
          <tr v-for="(visita) in visitas" :key="visita.id"
            ...
            <td class="px-6 py-4 font-medium text-green-900 whitespace-nowrap dark:text-white">
              <button @click="removerVisita(visita.id)" class="focus:outline-none">
