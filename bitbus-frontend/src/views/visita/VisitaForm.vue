    ...
    try {
      if (route.params.id) {
        const response = await axios.put(
          `http://127.0.0.1:8000/api/visita/${route.params.id}`,
          data
        );
        if (response) {
        ...
        const response = await axios.post(
          "http://127.0.0.1:8000/api/visita",
          data
        );
        if (response) {
          router.push({ name: "Visitas" });
    ...
    <div class="max-w-5xl grid mx-auto grid-cols-12">
      ...
      <div
        ...
        <table
          ...
          <tbody>
            <tr
              v-for="(visitante, index) in visitantesLista"
              :key="index"
              class="bg-white border-b dark:bg-green-800 dark:border-green-500 hover:bg-green-50 dark:hover:bg-green-600">
              <th
                scope="row"
