<script setup>
  ...
  const fetchOficina = async (id) => {
    const response = await axios.get(`http://127.0.0.1:8000/api/oficina/${id}`);
  ...
  onMounted(async () => {
    try {
      ...
      if (route.params.id) {
        await fetchOficina(route.params.id);
      }
    } catch (error) {
    ...
    try {
      if (route.params.id) {
        const response = await axios.put(
          `http://127.0.0.1:8000/api/oficina/${route.params.id}`,
          data
        );
        if (response) {
          router.push({ name: "Oficinas" });
        }
      } else {
        const response = await axios.post(
          "http://127.0.0.1:8000/api/oficina",
          data
        );
        if (response) {
          router.push({ name: "Oficinas" });
      ...
      console.error("Houve um erro ao adicionar a oficina:", error);
    }
  }
</script>
