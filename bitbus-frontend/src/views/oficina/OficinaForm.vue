...
async function handleSubmit(data) {
    ...
    try {
        if (route.params.id) {
            const response = await axios.put(`http://127.0.0.1:8000/api/oficina/${route.params.id}`, data);
            if (response) {
                router.push({ name: 'Oficinas' });
            }
        } else {
            const response = await axios.post('http://127.0.0.1:8000/api/oficina', data);
            if (response) {
                router.push({ name: 'Oficinas' });
            }
        }
...
<template>
    ...
    <div class="p-10 sm:ml-64 relative overflow-x-auto ">
        ...
        <div class="max-w-5xl grid mx-auto grid-cols-12">
            <h1 class="col-span-12 text-3xl text-green-900 dark:text-white mb-5">Cadastrar Oficina</h1>
            ...
            <h2 class="col-span-12 text-2xl text-green-900 dark:text-white my-5">Participantes</h2>
