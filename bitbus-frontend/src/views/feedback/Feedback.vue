<script setup>
  ...
  const buscarFeedback = () => {
    axios
      .get("http://127.0.0.1:8000/api/feedback?avaliacao=" + buscar.value)
      .then((response) => {
        feedbacks.value = response.data.data;
      })
      .catch((error) => {
...
<template>
  ...
  <div class="flex flex-col top-3.5 sm:ml-64 relative overflow-x-auto pr-10">
    ...
    <div class="flex justify-between">
      <div class="mx-4 flex space-y-4 sm:space-y-0">
        <label
          for="table-search"
    ...
    <div class="m-4 rounded-lg overflow-hidden border border-green-300">
      <table
        class="w-full text-sm text-left rtl:text-right text-green-500 dark:text-green-400">
        <thead
          class="text-xs text-green-700 uppercase bg-green-50 dark:bg-green-700 dark:text-green-50">
          <tr>
