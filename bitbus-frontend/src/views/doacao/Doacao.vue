...
onMounted(async () => {
  try {
    const response = await axios.get('http://127.0.0.1:8000/api/doacao');
...
const editarDoacao = (id) => {
  router.push({ name: 'DoacaoFormEdit', params: { id } });
}

async function removerDoacao(id) {
  try {
    const response = await axios.delete('http://127.0.0.1:8000/api/doacao/' + id);
    if (response) {
      console.error('Doação removida com sucesso');
      window.location.reload();

    }
  } catch (error) {
    console.error('Houve um erro ao remover a doação:', error);
  }
          ...
          <tr v-for="(doacao) in doacoes" :key="doacao.id"
            ...
            <td class="px-6 py-4">
              <div class="flex items">
                <button type="button" @click="editarDoacao(doacao.id)">Editar</button>
              </div>
            </td>
            <td class="px-6 py-4 font-medium text-teal-900 whitespace-nowrap dark:text-white">
              <button @click="removerDoacao(doacao.id)" class="focus:outline-none">
